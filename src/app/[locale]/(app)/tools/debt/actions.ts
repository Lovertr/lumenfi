...
interface PayCycleSnap {
  day: number;
  cycle_label: string;
  cycle_range: string;
  this_cycle_income: number;
  this_cycle_expense: number;
  this_cycle_net: number;
  avg_cycle_income: number;
  avg_cycle_expense: number;
...
interface FinancialSnapshot {
  ...
  pay_cycle?: PayCycleSnap | null;
  fallback_avg_income?: number;
  fallback_avg_expense?: number;
...
- ✅ **ลำดับการโปะเพิ่ม** (เงินส่วนเกินจากขั้นต่ำไปทางไหนก่อน):
  * ระบุชัดว่า "ทุกก้อนยังจ่ายขั้นต่ำตามตารางเดิมเสมอ" ก่อนพูดเรื่องโปะ
  * บอกว่า "เงินส่วนที่เหลือ" (เงินเพิ่ม/โบนัส/รายได้พิเศษ) ทุ่มไปก้อนไหนก่อน
  * เรียงลำดับเป็นข้อๆ พร้อมเหตุผล
- ⚠️ ระบุชัดว่าก้อนไหนเป็นหนี้มีหลักประกัน — ขั้นต่ำห้ามขาดงวดเด็ดขาด
...
═══════════════════════════════════════════════════════════
📦 ส่วนสุดท้ายของคำตอบ — บังคับใส่ JSON block (สำหรับ UI)
═══════════════════════════════════════════════════════════

หลังจาก markdown analysis ทั้งหมดข้างบน — **ต้องปิดท้ายด้วย JSON code block**
ที่มี 1-3 แผนการชำระหนี้ให้ผู้ใช้เลือก รูปแบบเป๊ะแบบนี้:
...
\`\`\`json
{
  "plans": [
    {
      "id": "plan-1",
      "title": "ชื่อแผนสั้นๆ (เช่น 'โปะดอกสูงก่อน + รวมหนี้บัตร')",
      "strategy": "avalanche | snowball | consolidate | refinance | debt_clinic | min_only",
      "extra_per_month": 0,
      "expected_months": 0,
      "total_interest": 0,
      "summary": "สรุปแผน 1-2 ประโยค บอกหลักการ",
      "steps": ["ขั้นตอนปฏิบัติ 1", "ขั้นตอน 2", "ขั้นตอน 3"],
...
กฎของ JSON block:
- มี 1-3 plans (เลือกที่เหมาะกับสถานการณ์)
- กำหนด \`recommended: true\` กับแผนที่ดีที่สุดเพียงแผนเดียว ที่เหลือ false
- ตัวเลข extra_per_month / expected_months / total_interest ต้องสมเหตุสมผลกับสถานการณ์
- ทุก field ต้องครบ ห้ามขาด
- JSON ต้อง valid (ห้ามมี trailing comma / comment)
- ใช้ภาษาไทยใน title/summary/steps/pros/cons
- ห้ามมีข้อความใดหลัง JSON block — จบที่ \`\`\``;

  // Use pay-cycle aware numbers if available
  const pc = snapshot.pay_cycle ?? null;
  const incomeForAnalysis = pc ? pc.avg_cycle_income : snapshot.monthly_income;
  const expenseForAnalysis = pc ? pc.avg_cycle_expense : snapshot.monthly_expense_total;
  const periodWord = pc ? `งวด (วันที่ ${pc.day})` : 'เดือน';

  const payCycleBlock = pc ? `

**🗓️ ข้อมูลตามรอบเงินเดือน (รอบ ${pc.cycle_label} · ${pc.cycle_range}):**
- รายรับงวดนี้: ฿${pc.this_cycle_income.toLocaleString()}
- รายจ่ายงวดนี้: ฿${pc.this_cycle_expense.toLocaleString()}
- สุทธิงวดนี้: ${pc.this_cycle_net >= 0 ? '+' : ''}฿${pc.this_cycle_net.toLocaleString()}

**📊 เฉลี่ย ${pc.cycles_analyzed} งวดที่ผ่านมา:**
- รายรับเฉลี่ย/งวด: ฿${pc.avg_cycle_income.toLocaleString()}
- รายจ่ายเฉลี่ย/งวด: ฿${pc.avg_cycle_expense.toLocaleString()}
- สุทธิเฉลี่ย/งวด: ${pc.avg_cycle_income - pc.avg_cycle_expense >= 0 ? '+' : ''}฿${(pc.avg_cycle_income - pc.avg_cycle_expense).toLocaleString()}
` : `

**📊 ข้อมูลเฉลี่ย ${snapshot.active_months} เดือนล่าสุด (รายเดือนปฏิทิน — ผู้ใช้ยังไม่ได้ตั้งรอบเงินเดือน):**
- รายรับเฉลี่ย/เดือน: ฿${snapshot.monthly_income.toLocaleString()}
- รายจ่ายเฉลี่ย/เดือน: ฿${snapshot.monthly_expense_total.toLocaleString()}
- สุทธิเฉลี่ย/เดือน: ${snapshot.monthly_income - snapshot.monthly_expense_total >= 0 ? '+' : ''}฿${(snapshot.monthly_income - snapshot.monthly_expense_total).toLocaleString()}
`;

  const objectionsBlock = objections.length > 0
    ? `\n\n**🚫 ข้อจำกัด / ข้อโต้แย้งที่ผู้ใช้แจ้งจากแผนรอบก่อน** (รอบที่ ${objections.length + 1}):
${objections.map((o, i) => `${i + 1}. ${o}`).join('\n')}

⚠️ สำคัญ: แผนใหม่ที่เสนอ **ห้ามขัดกับข้อจำกัดด้านบน** ต้องออกแบบทางเลือกที่ทำได้จริงภายในเงื่อนไขที่ผู้ใช้บอก
ถ้าทำไม่ได้จริงๆ ให้ระบุชัดในแผนว่าทำไมและเสนอทางเลือกอื่น`
    : '';
...
${payCycleBlock}
**สรุปการเงิน:**
- ผ่อนหนี้/${periodWord}: ฿${snapshot.existing_debt_payments.toLocaleString()}
- เงินเหลือใช้/${periodWord}: ฿${(incomeForAnalysis - expenseForAnalysis).toLocaleString()}
- DTI: ${dti.toFixed(1)}%
- หนี้รวม: ฿${snapshot.total_debt.toLocaleString()} (${monthsToCover.toFixed(1)} เท่าของรายได้/${periodWord})
...
**จ่ายเพิ่ม/เดือน:** ${extra_unspecified ? '🤖 ไม่ได้ระบุ — ขอให้คุณคำนวณและแนะนำจำนวนที่เหมาะสมที่สุดสำหรับผู้ใช้ จากเงินเหลือใช้และเป้าหมาย Cash Flow' : extra_per_month === 0 ? '฿0 (ผู้ใช้ยืนยันว่าไม่จ่ายเพิ่ม — แนะนำเฉพาะกลยุทธ์โดยใช้เงินที่ผ่อนปกติเท่านั้น)' : `฿${extra_per_month.toLocaleString()}`}${objectionsBlock}
